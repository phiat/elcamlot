...
# Use DATABASE_URL if set (e.g. from scripts/dev.sh), otherwise resolve Incus container IP
...
config :carscope, CarscopeWeb.Endpoint,
  ...
  watchers: [
    esbuild: {Esbuild, :install_and_run, [:carscope, ~w(--sourcemap=inline --watch)]},
    tailwind: {Tailwind, :install_and_run, [:carscope, ~w(--watch)]}
  ]
...
config :carscope, CarscopeWeb.Endpoint,
  live_reload: [
    ...
    patterns: [
      ...
      ~r"lib/carscope_web/router\.ex$"E,
      ~r"lib/carscope_web/(controllers|live|components)/.*\.(ex|heex)$"E
    ]
  ]
