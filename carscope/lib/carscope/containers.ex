defmodule Carscope.Containers do
  @moduledoc """
  ...
  @pg_container "carscope-pg"
  @ocaml_container "carscope-ocaml"
  ...
  @doc "List all CarScope containers."
  def list_containers do
    case incus(["list", "--format", "csv", "-c", "ns4"]) do
      {output, 0} ->
        ...
        |> String.trim()
        |> String.split("\n", trim: true)
        |> Enum.filter(&String.starts_with?(&1, "carscope-"))
        |> Enum.map(fn line ->
  ...
  def pg_ready?(name \\ @pg_container) do
    case exec(name, "pg_isready -U carscope") do
      {:ok, _} -> true
      _ -> false
    end
  end
  ...
  @doc "Tear down all CarScope containers."
  def teardown_all do
  ...
  defp project_root do
    # Navigate up from carscope/ to tc-lander/
    Application.app_dir(:carscope)
    |> Path.join("../../..")
    |> Path.expand()
  end
end
