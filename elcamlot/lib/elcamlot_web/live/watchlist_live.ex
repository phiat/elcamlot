defmodule ElcamlotWeb.WatchlistLive do
  use ElcamlotWeb, :live_view

  alias Elcamlot.Watchlist
  ...
  def mount(_params, _session, socket) do
    ...
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Elcamlot.PubSub, "user:#{user.id}:alerts")
    end
  ...
  def handle_event("run-now", %{"id" => id}, socket) do
    %{saved_search_id: id}
    |> Elcamlot.Workers.SearchScraperWorker.new()
    |> Oban.insert()
  ...
  def handle_event("delete-alert", %{"id" => id}, socket) do
    user = socket.assigns.current_scope.user
    alert = Elcamlot.Repo.get!(Elcamlot.Watchlist.PriceAlert, id)
