defmodule Elcamlot.BraveSearchTest do
  use ExUnit.Case, async: true

  alias Elcamlot.BraveSearch
  ...
  describe "parse_car_results/1" do
    ...
    test "returns error when API key missing" do
      original = Application.get_env(:elcamlot, :brave_search_api_key)
      Application.put_env(:elcamlot, :brave_search_api_key, nil)

      assert {:error, :missing_api_key} = BraveSearch.search("test")

      Application.put_env(:elcamlot, :brave_search_api_key, original)
    end
  end
end
